Feature: The Stories Endpoint
  ...
  Scenario: Fresh stories should be available
    When John retrieves the stories with the following details:
      | text                     | language | published_at.start | published_at.end | per_page | cursor | sort_by      | sort_direction |
      | corona, startup, ireland | en       | NOW-5MINUTES       | NOW              | 100      | *      | published_at | desc           |
